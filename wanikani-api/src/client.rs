...
#[cfg(feature = "study_material")]
mod study_material {
    use crate::{study_material::{StudyMaterial, CreateStudyMaterial, UpdateStudyMaterial}, Collection, Error, Resource};
    ...
    impl WKClient {
        ...
        pub async fn update_study_material(&self, id: u64, material: &UpdateStudyMaterial) -> Result<Resource<StudyMaterial>, Error> {
            let mut url = self.base_url.clone();
            url.path_segments_mut()
                .expect("Valid URL")
                .push(STUDY_MATERIAL_PATH)
...
#[cfg(feature = "reset")]
mod reset {
    use crate::{Collection, reset::Reset, Error, Resource};

    use super::{IdFilter, Filter, WKClient};
    ...
    impl WKClient {
        ...
        /// `created_at`, 500 at a time.
        pub async fn get_resets(
            &self,
            filters: &IdFilter,
        ) -> Result<Collection<Reset>, Error> {
            let mut url = self.base_url.clone();
            url.path_segments_mut()
                .expect("Valid URL")
                .push(RESET_PATH);
        ...
        pub async fn get_specific_reset(
            &self,
            id: u64,
        ) -> Result<Resource<Reset>, Error> {
            let mut url = self.base_url.clone();
            url.path_segments_mut()
                .expect("Valid URL")
                .push(RESET_PATH)
